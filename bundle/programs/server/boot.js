!function (module1) {
	...
	const meteorDebugPromise = process.env.METEOR_INSPECT_BRK
		? new Promise((resolve) => (meteorDebugPromiseResolver = resolve))
		: null;
		...
		if (meteorDebugPromise && meteorDebugPromiseResolver) {
			...
			setTimeout(async function poll() {
				...
				if (pauseStartTimeMs - waitStartTimeMs > waitLimitMs) {
					console.error(
						'Debugger did not attach after '.concat(
							waitLimitMinutes,
							' minutes; continuing.',
						),
					);
	...
	serverJson.load.forEach(function (fileInfo) {
		if (fileInfo.sourceMap) {
			...
			parsedSourceMaps[path.resolve(__dirname, fileInfo.path)] =
				parsedSourceMap;
		}
	});
	...
	const loadServerBundles = Profile('Load server bundles', async function () {
		...
		for (const fileInfo of serverJson.load) {
					...
					function (name, error) {
						if (nonLocalNodeModulesPaths.length > 0) {
							...
							nonLocalNodeModulesPaths.some(function (nodeModuleBase) {
								const packageBase = files.convertToOSPath(
						...
						throw (
							error || new Error('Cannot find module ' + JSON.stringify(name))
						);
					},
				),
			};
			function getAsset(assetPath, encoding, callback) {
				...
				// to.)
				const _callback = Package.meteor.Meteor.bindEnvironment(
					function (err, result) {
						if (result && !encoding)
			...
			const scriptPath = parsedSourceMaps[absoluteFilePath]
				? absoluteFilePath
				: fileInfoOSPath;
	...
	var callStartupHooks = Profile(
		'Call Meteor.startup hooks',
		async function () {
